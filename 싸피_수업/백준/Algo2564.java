 ...
 */
public class Algo2564 {
	static int N,M,T;
	static int[][] map;
	static int[] cx = {-1,0,1,0};
	static int[] cy = {0,-1,0,1};
	static int[] rcx = {1,0,-1,0};
	static int[] rcy = {0,-1,0,1};
	...
	public static void main(String[] args) {
		...
		for(int i=0;i<T;i++) {
			...
			if(a==1) {
				que.add(new Pair(0,b));
			}else if(a==2) {
				que.add(new Pair(N,b));
			}else if(a==3) {
				que.add(new Pair(b,0));
			}else {
				que.add(new Pair(b,M));
			}
		}
		
		int one = sc.nextInt();
		int two = sc.nextInt();
		Pair start = new Pair(two,two);
		if(one == 1) {
			start.first = 0;
		}else if(one == 2) {
			start.first = N;
		...
		while(!que.isEmpty()) {
			int queOne = que.peek().first;
			int queTwo = que.peek().second;
			que.poll();
			sum += Math.min(clock(start.first,start.second,one,queOne,queTwo), rClock(start.first,start.second,one,queOne,queTwo));
		}
		System.out.println(sum);
	}
	
	static public int clock(int startY, int startX, int a,int y, int x) {
		int cnt = 0;
		int ans = 0;
		if(a==1) {
			cnt = 2;
		...
		while(true) {
			ans++;
			if(startX+cx[cnt]<0 || startX+cx[cnt]>M || startY+cy[cnt]<0 || startY+cy[cnt]>N) {
				cnt = (cnt+1)%4;
			}
	...
	static public int rClock(int startY, int startX, int a,int y, int x) {
		int cnt = 0;
		int ans = 0;
		if(a==1) {
			cnt = 2;
		...
		while(true) {
			ans++;
			if(startX+rcx[cnt]<0 || startX+rcx[cnt]>M || startY+rcy[cnt]<0 || startY+rcy[cnt]>N) {
				cnt = (cnt+1)%4;
			}
